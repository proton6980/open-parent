package com.open.starter.mybatisplus.core.mapper;
...
public interface IBaseMapper<E extends BaseEntity, V> extends BaseMapper<E> {
     ...
     * <p>
     * 该方法使用反射机制从当前类（继承自 IBaseMapper 类）的泛型参数中获取第一个泛型类型 V 的 Class 对象
     *
     * @return 当前类的泛型类型 V 的 Class 对象
     */
    default Class<V> currentVoClass() {
        return (Class<V>) ReflectionKit.getSuperClassGenericType(this.getClass(), IBaseMapper.class, 1);
    }

    /**
     * 获取当前类的泛型类型 T 的 Class 对象
     * <p>
     * 该方法使用反射机制从当前类（继承自 IBaseMapper 类）的泛型参数中获取第一个泛型类型 T 的 Class 对象
     *
     * @return 当前类的泛型类型 T 的 Class 对象
     */
    default Class<E> currentModelClass() {
        return (Class<E>) ReflectionKit.getSuperClassGenericType(this.getClass(), IBaseMapper.class, 0);
    }
     ...
     */
    default boolean insertOrUpdateBatch(Collection<E> entityList) {
     ...
     * @return 更新操作是否成功的布尔值
     */
    default boolean updateBatchById(Collection<E> entityList, int batchSize) {
     ...
     * @return 插入或更新操作是否成功的布尔值
     */
    default boolean insertOrUpdateBatch(Collection<E> entityList, int batchSize) {
    ...
    default <C> List<C> selectVoByMap(Map<String, Object> map, Class<C> voClass) {
        List<E> list = this.selectByMap(map);
        if (CollUtil.isEmpty(list)) {
            return CollUtil.newArrayList();
        }
        return MapstructUtils.convert(list, voClass);
     ...
     * @return 查询到的单个VO对象
     */
    default V selectVoOne(Wrapper<E> wrapper) {
     ...
     * @return 查询到的单个VO对象
     */
    default V selectVoOne(Wrapper<E> wrapper, boolean throwEx) {
     ...
     */
    default <C> C selectVoOne(Wrapper<E> wrapper, Class<C> voClass) {
        E obj = this.selectOne(wrapper);
        if (Objects.isNull(obj)) {
            return null;
        }
        return MapstructUtils.convert(obj, voClass);
     ...
     * @return 查询到的单个VO对象，经过转换为指定的VO类后返回
     */
    default <C> C selectVoOne(Wrapper<E> wrapper, Class<C> voClass, boolean throwEx) {
        E obj = this.selectOne(wrapper, throwEx);
        if (Objects.isNull(obj)) {
            return null;
        }
        return MapstructUtils.convert(obj, voClass);
    ...
    default <C, P extends IPage<C>> P selectVoPage(IPage<E> page, Wrapper<E> wrapper, Class<C> voClass) {
        // 根据条件分页查询实体对象列表
        List<E> list = this.selectList(page, wrapper);
     ...
     */
    default <C> List<C> selectObjs(Wrapper<E> wrapper, Function<? super Object, C> mapper) {
