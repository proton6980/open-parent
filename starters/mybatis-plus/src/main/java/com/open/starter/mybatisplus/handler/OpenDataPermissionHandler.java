...
import com.open.starter.mybatisplus.enums.DataScopeType;
...
public class OpenDataPermissionHandler {
     ...
     */
    public OpenDataPermissionHandler(String mapperPackage) {
    ...
    public Expression getSqlSegment(Expression where, String mappedStatementId, boolean isSelect) {
        try {
            ...
            throw new OpenBusinessException("数据权限解析异常 => " + e.getMessage());
        } finally {
     ...
     * @throws OpenBusinessException 如果角色的数据范围异常或者 key 与 value 的长度不匹配，则抛出 OpenException 异常
     */
    private String buildDataFilter(DataPermission dataPermission, boolean isSelect) {
        ...
        for (DataColumn dataColumn : dataPermission.value()) {
            if (dataColumn.key().length != dataColumn.value().length) {
                throw new OpenBusinessException("角色数据范围异常 => key与value长度不匹配");
            }
            // 包含权限标识符 这直接跳过
            ...
            keys.addAll(Arrays.stream(dataColumn.key()).map(key -> "#" + key).collect(Collectors.toList()));
        }
        ...
        for (RoleDTO role : user.getRoles()) {
            ...
            if (ObjectUtil.isNull(type)) {
                throw new OpenBusinessException("角色数据范围异常 => " + role.getDataScope());
            }
            // 全部数据权限直接返回
            ...
            for (DataColumn dataColumn : dataPermission.value()) {
                ...
                // 不包含 key 变量 则不处理
                if (!StringUtils.containsAny(type.getSqlTemplate(), keys.toArray(new String[0]))) {
                    continue;
                }
                // 当前注解不满足模板 不处理
