package com.open.starter.satoken.aspect;
...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            // 发布事件保存数据库
            SpringUtil.publishEvent(operLog);
        } catch (Exception exp) {
            // 记录本地异常日志
    ...
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, OperateLogEvent operLog, Object jsonResult) throws Exception {
        ...
        if (log.isSaveResponseData() && ObjectUtil.isNotNull(jsonResult)) {
            operLog.setJsonResult(StringUtils.substring(JacksonUtils.toJsonString(jsonResult), 0, 2000));
        }
    }
    ...
    private String argsArrayToString(Object[] paramsArray, String[] excludeParamNames) {
        ...
        for (Object o : paramsArray) {
            if (ObjectUtil.isNotNull(o) && !isFilterObject(o)) {
                String str = JacksonUtils.toJsonString(o);
                Dict dict = JacksonUtils.parseMap(str);
                if (MapUtil.isNotEmpty(dict)) {
                    MapUtil.removeAny(dict, EXCLUDE_PROPERTIES);
                    MapUtil.removeAny(dict, excludeParamNames);
                    str = JacksonUtils.toJsonString(dict);
                }
                params.add(str);
            }
        }
