package com.open.extend.manager.function.tenantpackage.service.impl;
...
import com.open.extend.manager.function.tenantpackage.domain.bo.SysTenantPackageBo;
...
import com.open.extend.manager.function.tenantpackage.service.ISysTenantPackageService;
...
@Service
public class SysTenantPackageServiceImpl extends ServiceImpl<ISysTenantPackageMapper, SysTenantPackage> implements ISysTenantPackageService {
    ...
    public List<SysTenantPackageVo> selectList() {
        return baseMapper.selectVoList(new LambdaQueryWrapper<SysTenantPackage>()
                .eq(SysTenantPackage::getEnable, true));
    }
    ...
    private LambdaQueryWrapper<SysTenantPackage> buildQueryWrapper(SysTenantPackageBo bo) {
        ...
        lqw.eq(Objects.nonNull(bo.getEnable()), SysTenantPackage::getEnable, bo.getEnable());
        lqw.orderByAsc(SysTenantPackage::getId);
        return lqw;
    }
    ...
    public boolean checkPackageNameUnique(SysTenantPackageBo bo) {
        boolean exist = baseMapper.exists(new LambdaQueryWrapper<SysTenantPackage>()
            ...
            .ne(ObjectUtil.isNotNull(bo.getId()), SysTenantPackage::getId, bo.getId()));
        return !exist;
    }
    ...
    public Boolean deleteWithValidByIds(Collection<Long> ids, Boolean isValid) {
        if(isValid){
            boolean exists = tenantMapper.exists(new LambdaQueryWrapper<SysTenant>().in(SysTenant::getId, ids));
            if (exists) {
                throw new RuntimeException("租户套餐已被使用");
            }
        }
