package com.open.extend.manager.function.role.service.impl;
...
import com.open.extend.manager.function.role.domain.bo.SysRoleBo;
...
public class SysRoleServiceImpl extends ServiceImpl<ISysRoleMapper, SysRole> implements ISysRoleService {
    ...
    public List<SysRoleVo> selectRolesAuthByUserId(Long userId) {
        ...
        // 使用HashSet提高查找效率
        Set<Long> userRoleIds = StreamUtils.toSet(userRoles, SysRoleVo::getId);
    ...
    public List<Long> selectRoleListByUserId(Long userId) {
        ...
        return StreamUtils.toList(list, SysRoleVo::getId);
    }
    ...
    public void checkRoleAllowed(SysRoleBo role) {
        ...
        if (ObjectUtil.isNull(role.getId())
            ...
            throw new RuntimeException("不允许使用系统内置管理员角色标识符!");
        }
        // 修改不允许修改 管理员标识符
        if (ObjectUtil.isNotNull(role.getId())) {
            ...
            if (!StringUtils.equals(sysRole.getRoleKey(), role.getRoleKey())) {
                if (StringUtils.equalsAny(sysRole.getRoleKey(), keys)) {
                    throw new RuntimeException("不允许修改系统内置管理员角色标识符!");
    ...
    public void checkRoleDataScope(Long roleId) {
        ...
        roleBo.setId(roleId);
        List<SysRoleVo> roles = this.selectRoleList(roleBo);
    ...
    public int updateRole(SysRoleBo bo) {
        ...
        if (Constants.DISABLE.equals(role.getEnable()) && this.countUserRoleByRoleId(role.getId()) > 0) {
            throw new RuntimeException("角色已分配，不能禁用!");
        }
        // 修改角色信息
     ...
     */
    @Override
    public int updateRoleStatus(Long roleId, Boolean enable) {
        if (!enable && this.countUserRoleByRoleId(roleId) > 0) {
            throw new RuntimeException("角色已分配，不能禁用!");
        }
    ...
    private int insertRoleMenu(SysRoleBo role) {
        ...
        if (!list.isEmpty()) {
            rows = roleMenuMapper.insert(list).size();
        }
        return rows;
    }
    ...
    private int insertRoleDept(SysRoleBo role) {
        ...
        if (!list.isEmpty()) {
            rows = roleDeptMapper.insert(list).size();
        }
        return rows;
    }
    ...
    public int insertAuthUsers(Long roleId, Long[] userIds) {
        ...
        if (CollUtil.isNotEmpty(list)) {
            rows = userRoleMapper.insert(list).size();
        }
        if (rows > 0) {
            cleanOnlineUserByRole(roleId);
        }
