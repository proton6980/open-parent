package com.open.extend.manager.function.menu.service.impl;
...
import com.open.extend.manager.function.rolemenu.domain.SysRoleMenu;
import com.open.extend.manager.function.tenantpackage.domain.SysTenantPackage;
import com.open.extend.manager.function.menu.domain.bo.SysMenuBo;
import com.open.extend.manager.function.menu.domain.vo.MetaVo;
import com.open.extend.manager.function.menu.domain.vo.RouterVo;
import com.open.extend.manager.function.menu.domain.vo.SysMenuVo;
import com.open.extend.manager.function.menu.mapper.ISysMenuMapper;
import com.open.extend.manager.function.role.mapper.ISysRoleMapper;
import com.open.extend.manager.function.rolemenu.mapper.ISysRoleMenuMapper;
import com.open.extend.manager.function.tenantpackage.mapper.ISysTenantPackageMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<ISysMenuMapper, SysMenu> implements ISysMenuService {
    ...
    public List<SysMenuVo> selectMenuList(SysMenuBo bo, Long userId) {
        ...
        if (LoginHelper.isSuperAdmin(userId)) {
            menuList = baseMapper.selectVoList(new LambdaQueryWrapper<SysMenu>()
                .like(StringUtils.isNotBlank(bo.getMenuName()), SysMenu::getMenuName, bo.getMenuName())
                .eq(Objects.nonNull(bo.getVisible()), SysMenu::getVisible, bo.getVisible())
                .eq(Objects.nonNull(bo.getEnable()), SysMenu::getEnable, bo.getEnable())
                .orderByAsc(SysMenu::getParentId)
                .orderByAsc(SysMenu::getOrderNum));
        } else {
            QueryWrapper<SysMenu> wrapper = Wrappers.query();
            wrapper.inSql("r.role_id", "select role_id from sys_user_role where user_id = " + userId)
                .like(StringUtils.isNotBlank(bo.getMenuName()), "m.menu_name", bo.getMenuName())
                .eq(Objects.nonNull(bo.getVisible()), "m.visible", bo.getVisible())
                .eq(Objects.nonNull(bo.getEnable()), "m.enable", bo.getEnable())
    ...
    public List<Long> selectMenuListByPackageId(Long packageId) {
        ...
        if (tenantPackage.getMenuCheckStrictly()) {
            parentIds = baseMapper.selectObjs(new LambdaQueryWrapper<SysMenu>()
                ...
                .in(SysMenu::getId, menuIds), x -> {return Convert.toLong(x);});
        }
        return baseMapper.selectObjs(new LambdaQueryWrapper<SysMenu>()
            .in(SysMenu::getId, menuIds)
            .notIn(CollUtil.isNotEmpty(parentIds), SysMenu::getId, parentIds), x -> {return Convert.toLong(x);});
    }
    ...
    public List<RouterVo> buildMenus(List<SysMenu> menus) {
        ...
        for (SysMenu menu : menus) {
            ...
            if (CollUtil.isNotEmpty(cMenus) && MenuType.DIRECTORY.equals(menu.getMenuType())) {
